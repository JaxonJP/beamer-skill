...
1. For `compile [file]`, first check whether a working local TeX compilation environment is available (`latexmk`, `xelatex`, or equivalent).
2. If local compilation is available, compile locally first.
3. If local compilation tools are missing, ask whether to install the missing local dependencies or go directly to USTC LaTeX remote compilation.
...
2. **USTC remote branch**: if local compilation tools are unavailable, ask whether to install the missing local dependencies or go directly to **USTC LaTeX** (`https://latex.ustc.edu.cn`). If the user does not care, recommend the remote branch.
