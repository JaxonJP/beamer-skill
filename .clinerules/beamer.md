...
Default host: `https://latex.ustc.edu.cn`. Keep the GitHub fork as `origin`, add the platform as a second remote (for example `ustc-latex`), and ask the user for the platform Git URL and token if missing. Main compilation does **not** require local TeX, but some enhanced validation/export helpers may still rely on local utilities such as `PyMuPDF`, `pdfinfo`, `pdf2svg`, or `fc-list`.
